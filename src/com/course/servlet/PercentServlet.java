...
public class PercentServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		//添加百分比
		if(method.equals("addPercent")){
			int cid=Integer.parseInt(request.getParameter("cid"));
			int per=Integer.parseInt(request.getParameter("percent"));
			
			Percent percent=new Percent();
			percent.setCid(cid);
			percent.setTimecard_part(per*10);
			percent.setHomework_part(100-per*10);
			
			percentDao.addPercent(percent);
			
			response.sendRedirect("teacher/percent.jsp?cid="+cid);
